...
----> 1 print(f'Again, temperature in Kelvin was: {temp_k}')
...
def print_temperatures():
  print(f'temperature in Fahrenheit was: t{emp_fahr}')
  print(f'temperature in Kelvin was: {temp_kelvin}')
...
~~~
print(f'original min, mean, and max are:  {numpy.min(data)} , {numpy.mean(data)},  {numpy.max(data)}')
...
~~~
print(f'std dev before and after: {numpy.std(data)}, {numpy.std(offset_data)}')
~~~
{: .language-python}
...
~~~
print(f'difference in standard deviations before and after:
